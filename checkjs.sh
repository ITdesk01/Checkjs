...
SHARE_ACTIVITY_ID		jd_wxShareActivity.js
EOF
				...
				if [ $(cat $tg_add | wc -l) = "0" ];then
					...
					do
						...
						if [ -z "$variable_script_num" ];then
							...
							SHARE_ACTIVITY_ID|jd_wxSecond_activityId|VENDER_ID|wish_appIdArrList|jd_mhurlList|comm_activityIDList|computer_activityId|jd_wdz_activityId|M_FOLLOW_SHOP_ARGV|VENDER_ID|PKC_TXGZYL|LUCK_DRAW_URL|DPLHTY|jd_cjhy_activityId|jd_zdjr_activityId|jd_wxShareActivity_activityId|jd_wxgame_activityId|jd_drawCenter_activityId|JD_Lottery)
								export $i
